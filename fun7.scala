...
//
//def sumViaDivConq(ints: IndexedSeq[Int]): Int =
...
sum(Seq(1,2,3,4,5))
//sumViaDivConq(IndexedSeq(1,2,3,4,5))
...
//    Par.get(Par.unit(sum(sumL))) + Par.get(Par.unit(sum(r)))
...
//  }
//
//def sumWithExplicitForking(ints: IndexedSeq[Int]): Par[Int] =
//  if (ints.size <= 1)
//    Par.unit(ints.headOption getOrElse 0)
//  else {
//    val (l,r) = ints.splitAt(ints.length / 2)
...
//println(equal(S)(a, fork(a)))     // causes deadlock