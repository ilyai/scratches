...
sealed abstract class STArray[S,A](implicit manifest: Manifest[A]) {
  ...
  def fill(xs: Map[Int,A]): ST[S,Unit] = ST(xs.values)
...
object Immutable {
  ...
  def partition[S](a: STArray[S,Int], l: Int, r: Int, pivot: Int) = for {
    vp <- a.read(pivot)
    _ <- a.swap(pivot,r)
    j <- STRef(l)
...
Mutable.quicksort(List(4,7,1,66,32,0))
Immutable.quicksort(List(4,7,1,66,32,0))
